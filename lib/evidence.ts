import { searchBackfillNote } from './search-text';
...
export async function caReports(products:Product[],filters:ReportFilters):Promise<ReportResult>{
  ...
  for(let side=0;side<products.length;side++){
    ...
    const rows=await db().prepare(`SELECT id,name,ingredients FROM cv_products WHERE gen=? AND EXISTS(SELECT 1 FROM json_each(?) term WHERE instr(CASE WHEN searchText='' THEN upper(name||char(10)||ingredients) ELSE searchText END,term.value)>0) ORDER BY name,id LIMIT 31 OFFSET ?`).bind(gen,JSON.stringify(names),(filters.page-1)*30).all<{id:number;name:string;ingredients:string}>();
  ...
  const pending=await db().prepare("SELECT 1 FROM cv_products WHERE gen=? AND searchText='' LIMIT 1").bind(gen).first();
  if(selected.some(s=>!s.length))return {...result<ReportSummary[]>([],[...(pending?[searchBackfillNote]:[]),'Select the corresponding entries in the Canada Vigilance drug dictionary. A report-dictionary name does not establish an exact DPD product, manufacturer, or formulation match.',canadaCaveat],'partial'),counts:[],countLabel:'Choose report dictionary matches',candidates,selectedCvIds:selected,sourceAsOf:state.coverage,page:filters.page,hasMore:candidateHasMore};
