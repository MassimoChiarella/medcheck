...
The daily GitHub Actions workflow runs at 07:17 UTC and is also manually dispatchable. **GitHub authentication must be restored, a repository chosen/connected, and secrets installed before activation.** Enable the verified schedule with public launch; private review alone does not authorize a public launch. `MEDCHECK_URL`, `MEDCHECK_IMPORT_TOKEN`, and (only for private review) `SITES_AUTHORIZATION` are repository secrets. No provider plan changes or paid upgrades are automated.
