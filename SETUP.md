...
## Unattended updates and recovery

Each check records its attempt, heartbeat, phase and outcome before downloading. The source directory distinguishes updated, unchanged, failed and interrupted checks from the source's own coverage date. A check without a heartbeat for ten minutes is interrupted. The previous successful dataset stays available. Redacted local check records are in `work/checks/`; GitHub Actions writes a per-source job summary from these records.

Canada Vigilance compares verified source bytes against the installed generation before rebuilding its index. Downloads use ETag or Last-Modified when supplied and perform a full byte recheck at least every seven days. DPD currently supplies no such validators, so its five bounded downloads are still needed to confirm an unchanged catalogue. Actions caches only public government downloads; a corrupt or missing cache is downloaded again. The cache is optional and subject to GitHub's normal eviction and allowance policies.

US refreshes process each distinct DailyMed SPL once per cycle, even when several strengths reference it. Each completed label is checkpointed. Rate-limit waits do not consume failure attempts, and failed labels remain pending. `MEDCHECK_REFRESH_SECONDS` defaults to 7200 (accepted range 10–10800); an incomplete cycle fails visibly and resumes on the next check.

For a failed local update, rerun `npm run data:update`. For hosted imports, rerun **Refresh medication sources** manually from the repository Actions page. Other source steps still run after an individual source fails; a failed maintenance preflight stops subsequent imports. Do not erase the existing database or archive bucket to retry. For low capacity, inspect `npm run data:maintenance` first; apply eligible cleanup and rerun. If protected evidence still fills the allowance, the capacity blocker needs an explicit hosting/storage decision.

Owners watching this repository can enable GitHub's built-in Actions notifications in their personal **Settings → Notifications → Actions** and choose failed-workflow email notifications. Those personal delivery preferences are not configured by this repository. No additional email service or patient-facing alert is installed. See [GitHub notification settings](https://docs.github.com/en/subscriptions-and-notifications/how-tos/managing-github-actions-notifications).
