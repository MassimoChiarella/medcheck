...
export async function POST(request:Request){
  ...
  try{
    ...
    if(action==='dpd'||action==='dpd-complete'){
      ...
      const run=await db().prepare("SELECT * FROM imports WHERE id=? AND source='dpd'").bind(b.id).first<{state:string;manifest:string;created:string;cutoff:string;estimatedBytes:number}>();
      ...
      await writeCapacity(Math.max(12_000_000,Math.ceil(run.estimatedBytes/2)));
    ...
    if(action==='retired'){
      ...
      const rows=await db().prepare("SELECT id FROM imports WHERE source='cv' AND (state='cleaning' OR (state='retired' AND id NOT IN(SELECT id FROM imports WHERE source='cv' AND state='retired' ORDER BY completed DESC,id DESC LIMIT 1)))").all();return Response.json({generations:rows.results});
    }
    ...
    if(action==='promote'){
      ...
      await db().batch([
        ...
        db().prepare('UPDATE imports SET state=\'active\',completed=?,error=NULL,reservedBytes=0 WHERE id=?').bind(now(),id),
    ...
    if(action==='cleanup'){
      ...
      if((await protectedImports()).includes(id))throw new Error('The active and previous successful report datasets are protected.');
